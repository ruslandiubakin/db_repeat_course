...
try:
        with old_engine.connect() as old_connection, new_engine.connect() as new_connection:
                ...
                old_connection.execute(text('DROP TABLE "ZNO_Places";'))
                old_connection.execute(text('DROP TABLE "Results_Of_Students";'))
                old_connection.execute(text('DROP TABLE "EI_of_Students";'))
                old_connection.execute(text('DROP TABLE "Subjects";'))
        ...
        # Закрийте з'єднання у блоку finally, щоб гарантувати їх закриття навіть у випадку помилки
